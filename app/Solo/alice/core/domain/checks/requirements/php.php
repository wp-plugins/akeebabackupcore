...
class AliceCoreDomainChecksRequirementsPhp extends AliceCoreDomainChecksAbstract
{
    ...
    {
        parent::__construct(10, Text::_('ALICE_ANALYZE_REQUIREMENTS_PHP_VERSION'), $logFile);
    }
		...
		while(($line = fgets($handle)) !== false)
		{
			...
			if($pos !== false)
			{
				...
				if(version_compare($version, '5.3', 'lt'))
				{
                    ...
                    $this->setResult(-1);
					throw new Exception(Text::sprintf('ALICE_ANALYZE_REQUIREMENTS_PHP_VERSION_ERR_TOO_NEW', $version));
				}
				/*
				elseif(version_compare($version, '5.5', 'ge'))
				{
                    ...
                    $this->setResult(-1);
					throw new Exception(Text::sprintf('ALICE_ANALYZE_REQUIREMENTS_PHP_VERSION_ERR_TOO_OLD', $version));
				}
				*/
	...
	{
		return Text::_('ALICE_ANALYZE_REQUIREMENTS_PHP_VERSION_SOLUTION');
	}
}
