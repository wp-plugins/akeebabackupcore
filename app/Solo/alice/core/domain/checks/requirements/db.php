...
class AliceCoreDomainChecksRequirementsDb extends AliceCoreDomainChecksAbstract
{
    ...
    {
        parent::__construct(20, Text::_('ALICE_ANALYZE_REQUIREMENTS_DATABASE'), $logFile);
    }
	...
	public function check()
	{
        ...
        if($connector == 'mysql' || $connector == 'mysqli')
        {
            ...
            {
                $this->setResult(-1);
                throw new Exception(Text::sprintf('ALICE_ANALYZE_REQUIREMENTS_DATABASE_VERSION_TOO_OLD', $version));
            }
        }
        elseif($connector == 'oracle')
        {
            $this->setResult(-1);
            throw new Exception(Text::sprintf('ALICE_ANALYZE_REQUIREMENTS_DATABASE_UNSUPPORTED', 'SqlAzure'));
        }
        elseif($connector == 'pdo')
        {
            $this->setResult(-1);
            throw new Exception(Text::sprintf('ALICE_ANALYZE_REQUIREMENTS_DATABASE_UNSUPPORTED', 'PDO'));
        }
        ...
        {
            $this->setResult(-1);
            throw new Exception(Text::sprintf('ALICE_ANALYZE_REQUIREMENTS_DATABASE_UNSUPPORTED', 'SQLite'));
        }
        else
        {
            // Unknown database type, throw exception
            $this->setResult(-1);
            throw new Exception(Text::sprintf('ALICE_ANALYZE_REQUIREMENTS_DATABASE_UNKNOWN'));
        }
	...
	{
		return Text::_('ALICE_ANALYZE_REQUIREMENTS_DATABASE_SOLUTION');
	}
}
