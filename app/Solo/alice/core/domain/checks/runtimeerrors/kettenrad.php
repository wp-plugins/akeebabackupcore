...
class AliceCoreDomainChecksRuntimeerrorsKettenrad extends AliceCoreDomainChecksAbstract
{
    ...
    {
        parent::__construct(10, Text::_('ALICE_ANALYZE_RUNTIME_ERRORS_KETTENRAD'), $logFile);
    }
	...
	public function check()
	{
        ...
        if(count($starting) != count($saving))
        {
            ...
            $this->setResult(-1);
            throw new Exception(Text::sprintf('ALICE_ANALYZE_RUNTIME_ERRORS_KETTENRAD_SAVING_DIFFERS', count($starting), count($saving)));
        }
        ...
        foreach($starting as $stepNumber)
        {
            ...
            if(count(array_keys($starting, $stepNumber)) > 1)
            {
                ...
                $this->setResult(-1);
                throw new Exception(Text::sprintf('ALICE_ANALYZE_RUNTIME_ERRORS_KETTENRAD_STARTING_MORE_ONCE', $stepNumber));
            }
        }
	...
	{
		return Text::_('ALICE_ANALYZE_RUNTIME_ERRORS_KETTENRAD_SOLUTION_1').
               Text::_('ALICE_ANALYZE_RUNTIME_ERRORS_KETTENRAD_SOLUTION_2');
	}
}
