...
class Manage extends ControllerDefault
{
	...
	public function showComment()
	{
		...
		if ($id <= 0)
		{
			$this->setRedirect($returnUrl, Text::_('STATS_ERROR_INVALIDID'), 'error');
		}
		else
		{
	...
	public function download()
	{
		...
		if ($id <= 0)
		{
			...
			$this->setRedirect($url, Text::_('STATS_ERROR_INVALIDID'), 'error');
		...
		if (is_null($fileName) || empty($fileName) || !@file_exists($fileName))
		{
			...
			$this->setRedirect($url, Text::_('STATS_ERROR_INVALIDDOWNLOAD'), 'error');
	...
	public function remove()
	{
		...
		$this->setRedirect($returnUrl, Text::_('STATS_MSG_DELETED'));
	}
	...
	public function deleteFiles()
	{
		...
		$this->setRedirect($returnUrl, Text::_('STATS_MSG_DELETEDFILE'));
	}
	...
	public function save()
	{
		...
		if ($result !== false)
		{
			$message = Text::_('STATS_LOG_SAVEDOK');
			$type = 'message';
		}
		else
		{
			$message = Text::_('STATS_LOG_SAVEERROR');
			$type = 'error';
		}
